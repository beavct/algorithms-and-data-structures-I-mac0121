...
void posfixa (char exp[]) {
    ...
    for (i = 0; exp[i] != '\0'; i++) {
        if (exp[i] == ' ') continue; /*caso não usuário coloque espaço em branco, 'continue' continua para a próxima interação do laço*/
        if (!operador(exp[i])) printf ("%c", exp[i]);
        else if (exp[i] == '(') empilha(pilhaOp, exp[i]);
        else if (exp[i] == ')') cTopo = desempilha(pilhaOp) {
            while (cTopo != '(') { 
                printf ("%c", cTopo);
                cTopo = desempilha(pilhaOp);
            }
        else { /*leu operador e tem que comparar a precedência */
